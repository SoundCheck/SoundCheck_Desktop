...
public class SongListManager implements Runnable {
	...
	public SongListManager(File parentDir, MasterController mc) {//, PlayerGUI gui) {
	...
	@Override
	public void run() {
		buildLocalSongList(parentDir, mc);//, gui);
	}
	 ...
	 * @return
	 */
	public static final void buildLocalSongList(File parentDir, MasterController mc){//, PlayerGUI gui) {
...
//		gui.controlPane.infoUpdate("Loading Songs...");
	 ...
	 * @return
	 */
	private static final List<Song> addFolderToList(File parentDir) {
		...
		for (File child : childFiles) {
			if (child.isFile() && child.getName().contains(".mp3")) {
				...
				songList.addAll(addFolderToList(child));
			}
		}
		return songList;
	}
